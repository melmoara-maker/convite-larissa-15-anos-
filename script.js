...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("keydown", (evento) => {
    ...
    if (modalPresentes?.classList.contains("aberto")) {
      fecharModalPresentes();
    }

    if (modalLocalizacao?.classList.contains("aberto")) {
      fecharModalLocalizacao();
    }
  });


  const abrirLocalizacao = document.querySelector("#abrirLocalizacao");
  const fecharLocalizacao = document.querySelector("#fecharLocalizacao");
  const voltarConvite = document.querySelector("#voltarConvite");
  const modalLocalizacao = document.querySelector("#modalLocalizacao");
  let posicaoAntesDaLocalizacao = 0;

  const abrirModalLocalizacao = () => {
    posicaoAntesDaLocalizacao = window.scrollY;
    modalLocalizacao?.classList.add("aberto");
    modalLocalizacao?.setAttribute("aria-hidden", "false");
    document.body.classList.add("modal-aberto");
    fecharLocalizacao?.focus();
  };

  const fecharModalLocalizacao = () => {
    modalLocalizacao?.classList.remove("aberto");
    modalLocalizacao?.setAttribute("aria-hidden", "true");
    document.body.classList.remove("modal-aberto");

    window.requestAnimationFrame(() => {
      window.scrollTo({ top: posicaoAntesDaLocalizacao, behavior: "smooth" });
      abrirLocalizacao?.focus();
    });
  };

  abrirLocalizacao?.addEventListener("click", abrirModalLocalizacao);
  fecharLocalizacao?.addEventListener("click", fecharModalLocalizacao);
  voltarConvite?.addEventListener("click", fecharModalLocalizacao);

  modalLocalizacao?.addEventListener("click", (evento) => {
    if (evento.target === modalLocalizacao) fecharModalLocalizacao();
  });
